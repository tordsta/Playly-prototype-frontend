function CGame(oData){
    ...
    this.startGame = function(){
        ...
        for (var i=0;i<NUM_PLAYERS;i++){
            ...
            _aPlayersHand[i] = new CHandDisplayer(iOffsetX,iOffsetY,HAND_POS["num_player_"+NUM_PLAYERS][i].x,HAND_POS["num_player_"+NUM_PLAYERS][i].y,i,_oHandsContainer,aTextPlayers[i]);
        }
    ...
    this.restart = function(){
        ...
        _oUsedCards.clearCards();
        _oUsedCards.disableInputDraw();

        for(var i=0; i<_aPlayersHand.length; i++){
            _aPlayersHand[i].clearCards();
        }
    ...
    this.applyEffectOnCard = function(oCard,bFirstCard){
        ...
        if (oCard.getFotogram()==="color"){
            ...
            if(iPlayer===0){
                ...
                _oTurnManager.nextTurn();
                _oAnimation.changeColor(_iCurrentColor);
            }
            ...
            if (oCard.getRank()===10){
                ...
                _oTurnManager.nextTurn();
                if (_bUNO===true){
                    setTimeout(function(){if (_bUNO===true){oParent.drawCards(0,2,0,bPenality);};setTimeout(function(){_oAnimation.stopTurn();},800);},2000);
                }else{
                    _oAnimation.stopTurn();
                }
                ...
                if (_bUNO===true){
                    setTimeout(function(){if (_bUNO===true){oParent.drawCards(0,2,0,bPenality)};setTimeout(function(){_oAnimation.changeClockWise(_oTurnManager.getClockWise());},1000);},2000);
                }else{    
                    _oAnimation.changeClockWise(_oTurnManager.getClockWise());
                }
                
            }else if(oCard.getRank()===12){
                if (_bUNO===true){
                    setTimeout(function(){if (_bUNO===true){oParent.drawCards(0,2,0,bPenality);};setTimeout(function(){_oAnimation.drawTwoAnim();},1000);},2000);
                }else{
                    _oAnimation.drawTwoAnim();
                }
    ...
    this.drawCardsTween = function(iIndexPlayer,iNumberOfCards,iDelay,bPenality){
        ...
        if (_oDeck.getLength()===0&&_bEntered===false){
            _bEntered = true;
            _oAnimation.shuffleAnimation(iIndexPlayer,iNumberOfCards,iDelay);
        }else if (iNumberOfCards===1){
